...
pub fn search_case_insensitive<'a>(query: &str, contents: &'a str) -> Vec<(usize,&'a str)>{
    ...
    for (c,line) in contents.lines().enumerate(){
...
Duct tape.";
        assert_eq!(
            ...
            search(query,contents).iter().map(|(_,s)| *s).collect::<Vec<&str>>()
        );
    }
...
Trust me.";
        assert_eq!(
            vec!["Rust:","Trust me."],
            search_case_insensitive(query,contents).iter().map(|(_,s)| *s).collect::<Vec<&str>>()
        );
    }
}
